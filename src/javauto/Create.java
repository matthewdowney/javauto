...
public class Create {
	...
	/* ArrayList to hold our struct objects, which are converted into
	 * java objects and must be written to a separate file in the same
	 * directory. Each index should have [struct name, struct code] */
	...
	public static void main(String[] args) {
		...
		if (args.length == 0) {
			...
			System.out.println("\t-g\t--generate\t\tgenerate .java and .class files");
			System.out.println("\t-gj\t--generate-java\t\tgenerate .java files");
			System.out.println("\t-gc\t--generate-class\tgenerate .class files");
		}
		...
		for (String userCodeFile: args) {
			...
			else if (userCodeFile.equals("-g") || userCodeFile.equals("--generate")) {
			...
			/* if it's not a file but a flag */
			else if (userCodeFile.equals("-gj") || userCodeFile.equals("--generate-java")) {
				/* set keeps to true and continue with next iteration */
				genJava = true;
				continue;
			}
			...
			/* if it's not a file but a flag */
			else if (userCodeFile.equals("-gc") || userCodeFile.equals("--generate-class")) {
				/* set keeps to true and continue with next iteration */
				keepClass = true;
				continue;
			}
			...
			if (!userCodeFile.toLowerCase().endsWith(".javauto") && !userCodeFile.toLowerCase().endsWith(".ja")) {
				System.out.println("Not a .javauto file:  " + GREEN + userCodeFile + NORMAL);
				continue;
			}

			/* check to make sure the file exists */
			if ( ! new File(userCodeFile).exists() ) {
				System.out.println("File not found:  " + GREEN + userCodeFile + NORMAL);
				continue;
			}

			/* the class name of our main user code */
			String className = new File(userCodeFile).getName().split("\\.(?=[^\\.]+$)")[0];

			verbose("Generating " + GREEN + className + NORMAL + "...");

			if (!className.matches("^[a-zA-Z].*")) {
				System.out.println(RED + "Error: class names must start with a letter. " + GREEN + className + RED + " does not start with a letter." + NORMAL);
				System.exit(1);
			}
			if (!className.matches("^\\S+$")) {
				System.out.println(RED + "Error: class names must not contain spaces. " + GREEN + className + RED + " contains spaces." + NORMAL);
				System.exit(1);
			}

			/* file path of our build folder (currentDir/build) */
			String directory = new File(userCodeFile).getAbsolutePath().substring(0, new File(userCodeFile).getAbsolutePath().lastIndexOf(File.separator)) + File.separator + className;

			/* if we're not keeping any of the generated files make the dir hidden */
			if (!keepClass && !genJava)
				directory = new File(userCodeFile).getAbsolutePath().substring(0, new File(userCodeFile).getAbsolutePath().lastIndexOf(File.separator)) + File.separator + "." + className;

			/* file path of our gen directory (build/gen) for generated java files */
			String genDirectory = directory + File.separator + "gen";

			/* file path of our class directory (build/class) for our compiled .class files */
			String classDirectory = directory + File.separator + "class";

			/* get javauto code to compile */
			String javautoCode = fileRead(userCodeFile);

			/* perform rudimentary debugging and stop compiling if there's an error */
			...
			if (!outDir.exists()) {
				...
				if (!outDir.mkdir()) {
					System.out.println("Error: couldn't create build directory " + GREEN + directory + NORMAL + "\nPlease create this directory manually.");
					System.exit(1);
				}
			}
			/* if our build folder is temporary and we're on windows hide the folder manually */
			if (!keepClass && !genJava && System.getProperty("os.name").toLowerCase().contains("windows")) {
				String cmd = "Executing cmd /c attrib +s +h \"" + directory + "\"...";
				verbose(cmd);
				try {
					Process p = Runtime.getRuntime().exec("cmd /c attrib +s +h \"" + directory + "\"");
				} catch (Exception e) {
			...
			if (genJava) {
				...
				if (!outDir.exists()) {
					verbose("Creating directory " + GREEN + genDirectory + NORMAL + "...");
					if (!outDir.mkdir()) {
						System.out.println("Error: couldn't create gen directory " + GREEN + genDirectory + NORMAL + "\nPlease create this directory manually.");
						System.exit(1);
					}
				}
			}
			...
			if (!outDir.exists()) {
				...
				if (!outDir.mkdir()) {
					System.out.println("Error: couldn't create class directory " + GREEN + classDirectory + NORMAL + "\nPlease create this directory manually.");
					System.exit(1);
				}
			}
			...
			if (genJava) {
				...
				for (String[] structFile : structFiles) {
					String outPath = genDirectory + File.separator + structFile[0] + ".java";
			...
			if (genJava) {
				/* build our main file */
				String outputFile = genDirectory + File.separator + className + ".java";
			...
			for (String[] structFile : structFiles) {
				verbose("Building " + GREEN + classDirectory + File.separator + structFile[0] + ".class" + NORMAL + "...");

				/* use our custom compiler (CustomJavaCompiler.java) to attempt to
				 * compile the file, if it doesn't work out errors will be stored in
				 * the DiagnosticCollector<JavaFileObject> */
				DiagnosticCollector<JavaFileObject> compileErrors = null;
				try {
					compileErrors = CustomJavaCompiler.compile(structFile[0], structFile[1], classDirectory, classDirectory);
				} catch (IOException ioe) {
					System.out.println(RED + "Error: couldn't write to file in directory " + classDirectory + NORMAL);
					ioe.printStackTrace();
				}
				...
				if (compileErrors != null) {
					...
					for (Diagnostic diagnostic : compileErrors.getDiagnostics()) {
						...
						/* find the struct in the user code that is causing the issue */
						String[] javautoCodeLines = javautoCode.split("\n");
						int structlinenum = -1;
						for (int k = 0; k < javautoCodeLines.length; k++) {
							if (removeLiterals(removeComments(javautoCodeLines[k])).trim().startsWith("struct " + structFile[0]))
								structlinenum = k;
						}
						...
						if (structlinenum == -1)
							...
							System.out.println(diagnostic.getKind().toString().toLowerCase() + " in struct " + structFile[0] + ":");
						else
							/* print the error message */
							System.out.println(diagnostic.getKind() + " in struct declaration for " + structFile[0] + " starting on line " + structlinenum + ":\n\t" + javautoCodeLines[structlinenum]);
					...
					if (!keepClass && !genJava) {
						if (deleteDirectory(new File(directory))) {
							verbose("Removed temporary dir " + GREEN + directory + NORMAL);
						} else {
							verbose(RED + "Failed to remove temporary dir " + directory + NORMAL);
						}
					}
			...
			verbose("Building " + GREEN + classDirectory + File.separator + className + ".class" + NORMAL + "...");
			...
			DiagnosticCollector<JavaFileObject> compileErrors = null;
			try {
				compileErrors = CustomJavaCompiler.compile(className, generatedCode, classDirectory, classDirectory);
			} catch (IOException ioe) {
				System.out.println(RED + "Error: couldn't write to file in directory " + classDirectory + NORMAL);
				ioe.printStackTrace();
			}
			...
			if (compileErrors != null) {
				...
				/* give the errors to our DealWithCompilerErrors.java to take care of */
				DealWithCompilerErrors.dealWithIt(compileErrors, generatedCode, javautoCode, userCodeFile);
			...
			try {
				...
				System.out.println(RED + "Error: could not write to file " + jarOut + NORMAL);
				ioe.printStackTrace();
			} finally {
				if (!keepClass && !genJava) {
					if (deleteDirectory(new File(directory))) {
			...
			verbose("Setting executable permissions for " + GREEN + jarOut + NORMAL + "...");
	 ...
	 * @param javautoCode raw javauto code to be converted
	 * @param userClassName the name of the class/output file
	 * @return java code as String
	 */
	private static String generateJavaCode(String javautoCode, String userClassName) {
		...
		for (int i = 0; i < jaCodeLines.length; i++) {
			if ( jaCodeLines[i].trim().startsWith("import ") ) {
				if (!alreadyImported(jaCodeLines[i].trim(), generatedImports)) {
					...
					verbose(YELLOW + jaCodeLines[i].trim().substring(jaCodeLines[i].trim().lastIndexOf(".")+1, jaCodeLines[i].trim().lastIndexOf(";")) + " " + NORMAL, 0);
					printedResults = true;
				}
				jaCodeLines[i] = "";
			}
		...
		/* insert ~most~ of our generated and user sourced code into the template */
		...
		generatedCode = generatedCode.replace("<userClassName>", "public class " + userClassName + " {");
		generatedCode = generatedCode.replace("<userGlobalVariables>", indent(userGlobalVariables));
		generatedCode = generatedCode.replace("<userCode>", "\tpublic static void main(String[] args) {\n" + userCode + "\n\t}\n");
		generatedCode = generatedCode.replace("<userFunctions>", indent(userFunctions));

		/* now we generate javauto functions based on what the user has implemented until no replacements have been made */
		int replacements = 1;
		...
		while (replacements != 0) {
			...
			for (String[] javautoFunction : javautoFunctions) {
				/* if a the function name + ( is present anywhere in our code AND we haven't already generated a function for it */
				if (noLiteralsCode.contains(javautoFunction[0].trim()) && !alreadyGenerated.contains(javautoFunction[0].trim())) {
					/* add code for that function to the file */
					generatedCode = generatedCode.replace("<{_*_generatedFunctions_*_}>", removeComments(javautoFunction[1]) + "\n<{_*_generatedFunctions_*_}>");
		...
		/* we've generated all the functions we need => remove the <generatedFunctions> tag */
		...
		generatedCode = generatedCode.replace("<generatedVariables>", usedVariables);
		...
		for (String line : generatedImports.split("\n")) {
			String importedClass = line.substring(line.lastIndexOf(".")+1, line.lastIndexOf(";"));
		...
		generatedCode = generatedCode.replace("<generatedImports>", generatedImports);
	...
	/**
	 * Searches user code for a struct, pieces it together,
	 * and returns the struct code and the new user code.
	 * @param code the user code
	 * @return String array
	 * 	index 0: new user code
	 *	index 1: the struct name
	...
	private static String[] getStruct(String code) {
		...
		for (int i = 0; i < codeLines.length; i++) {
			...
			if (removeComments(codeLines[i]).trim().startsWith("struct ")) {
				/* increment the start index until we're at the beginning of the declaration */
				while (inIgnoredText(startIndex, allIgnores) || codeChars[startIndex] != 's')
					startIndex++;
				...
				while (endIndex < codeChars.length) {
					if (codeChars[endIndex] == '{' && !inIgnoredText(endIndex, allIgnores)) {
						break;
					}
					endIndex++;
				}
				...
				while (endIndex < codeChars.length) {
					...
					insideElements = insideElements + String.valueOf(codeChars[endIndex]);
					endIndex++;
				}
				endIndex++;

				/* get the user code without the struct object */
				String newCode = code.substring(0, startIndex) + code.substring(endIndex, codeChars.length);

				/* get the name of the struct object to create */
				String structName = removeComments(code.substring(startIndex, endIndex)).split(" ")[1].trim();
				...
				for (int j = 0; j < insideElementsLines.length; j++) {
					insideElementsLines[j] = removeComments(insideElementsLines[j]).trim();
				}
				insideElements = "";
				...
				elementCode = elementCode.substring(0, elementCode.length()-1);
	 ...
	 * @param userCode the text from which to extract the function
	 * @param lowerBound the start index of the function
	...
	private static int getFunctionUpperBound( String userCode, int lowerBound ) {
		...
		while (open > close) {
			if (codeChars[endIndex] == '}' && !inIgnoredText(endIndex, allIgnores)) {
				close++;
			} else if (codeChars[endIndex] == '{' && !inIgnoredText(endIndex, allIgnores)) {
				open++;
			}
			endIndex++;
		}
	...
	/**
	 * Split user code into an array; one indice will contain
	 * user functions and the other will contain all other
	 * user code.
	 * @param userCode
	...
	private static String[] splitUserFunctions(String userCode) {
		...
		for (String line : userCode.split("\n")) {
			...
			if (line.trim().startsWith("func ")) {
				int[] bounds = {charIndex, getFunctionUpperBound(userCode, charIndex)};	
		...
		for (int[] bounds : functionIndexes) {
			/* append what is outside the function bounds to the userNormalCode variable */
			userNormalCode = userNormalCode + userCode.substring(nonFunctionIndex, bounds[0]) + "\n";

			/* get the current user function & change its first line for java func decalration */
			String[] functionLines = userCode.substring(bounds[0], bounds[1]).split("\n");
			...
			verbose(YELLOW + restoff.substring(0, restoff.indexOf("(")) + " " + NORMAL, 0);
			printedResults = true;
			...
			/* append what is inside the function bounds to the userFunctions variable */
			userFunctions = userFunctions + function + "\n\n";

			/* advance the lower bound of the "non function" code to the upper bound of the function code */
			nonFunctionIndex = bounds[1];
		}

		/* add any remaining code to the user "non-function" code */
		userNormalCode = userNormalCode + userCode.substring(nonFunctionIndex, userCode.length());
	...
	/**
	 * Search the javautoFile for imports and add them all to the generated imports
	...
	/**
	 * Handle all python like string replacements ("%s" % something)
	 * within a string.
	 * @param data Data within which replacements happen.
	 * @return Data with all replacements done.
	 */
	private static String doPythonStringReplacements(String data) {
		/* As long as some are being replaced keep replacing them.
		 * We have to do this one at a time because as soon as one
		 * part is changed the indexes to ignore (string literals
		 * and comments) change so we have to rebuild everything
		 */
	...
	/**
	 * Handle a _single_ python style string replacement within a string.
	 * For instance, the following string:
	 * System.out.println("Time: \t%s" % (time));
	 * Would become:
	 * System.out.println("Time: \t%s".replaceFirst("%s",time));
	 * @param data the string to search through and perform a replacement in.
	 * @return a string array, the first index being the modified string data,
	 * the second index being either "some" or "none", depending on if a replacement
	 * was made or if no replacement was made.
	 */
	private static String[] doPythonStringReplacement(String data) {
		...
		while (matcher.find()) {
			 ...
			 * because we can check whether it's inside a comment/string to
			 * see if we should actually do the modification.
			 */
			...
			if (!inIgnoredText(checkIndex, allIgnores)) { 
				/* find the upper bound of the elements we're adding into the string */
				int elementsEndIndex = findEndParen(elementsStartIndex, data, allIgnores);

				/* get the elements that need to be subbed in */
				ArrayList<String> elements = getElements(elementsStartIndex, elementsEndIndex, data, allIgnores);
				...
				String modString = data.substring(matchedStartIndex, literalEndIndex) + rCode;

				/* build a new string with the replacement */
				String fixed = data.substring(0,matchedStartIndex) + modString + data.substring(elementsEndIndex);
	...
	/**
	 * Get each element inside of a tuple like structure.
	 * So for a given line of code:
	 * String intro = "My name is %s %s %s" % ("John", getMiddleName("John"), getLastName("John"));
	 * The tuple-like structure               ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
	 * The element list would look like ["\"John\"", "getMiddleName(\"John\")", "getLastName(\"John\")"]
	 * @param startElement the index of the opening ( of the tuple-like structure
	 * @param endElements the index of the closing ) of the tuple-like structure
	 * @param context the greater string in which it resides
	 * @param indexesToIgnore a list of index bounds we want to ignore, eg comments or
	 * string literals. Each element should have the format [startOfIgnore, endOfIgnore]
	 * @return a list of individual elements
	 */
	private static ArrayList<String> getElements(int startElement, int endElement, String context, ArrayList<Integer[]> indexesToIgnore) {
		/* get the comma indexes */
		ArrayList<Integer> splitIndexes = getElementCommas(startElement,endElement,context,indexesToIgnore);
		...
		if (splitIndexes.size() > 0) {
			for (int i = 1; i < splitIndexes.size(); i++) {
				elements.add(context.substring(splitIndexes.get(i-1)+1, splitIndexes.get(i)).trim());
			}
		}
	...
	/**
	 * Very specific use. Gets the "real" commas inside of a tuple-like structure.
	 * This shows which indexes it would return for an example:
	 * ("parameter1", someFunction("foo", getData("foo", "bar")), "parameter3")
	 *              ^                                           ^              
	 * @param startElement the index of the beginning of the tuple like element
	 * @param endElement the index of the end of the tuple like element
	 * @param context the String within which this "tuple" is contained
	 * @param indexesToIgnore a list of index bounds we want to ignore, eg comments or
	 * string literals. Each element should have the format [startOfIgnore, endOfIgnore]
	 * @return a List of indexes where "true" commas are present
	 */
	private static ArrayList<Integer> getElementCommas(int startElement, int endElement, String context, ArrayList<Integer[]> indexesToIgnore) {
		...
		while (index < endElement) {
			/* if it's a comma and it's not inside our comments/literals add it to the list */
			if (contextChars[index] == ',' && !inIgnoredText(index,indexesToIgnore)) {
				realCommas.add(index);
				index++;
			}
			/* if it's an open paren but it isn't inside a comment/literal skip to the end paren */
			else if (contextChars[index] == '(' && !inIgnoredText(index, indexesToIgnore)) {
	 ...
	 * @param startParen the index of the '('.
	 * @param context the string within which the parens are located.
	 * @param indexesToIgnore A list of Integers like [start, end] that specify indexes
	 * that should be ignored, this list usually contains the start and end bounds of 
	 * each comment and string literal in the context.
	 * @return the index of the closing paren
	 */
	private static int findEndParen(int startParen, String context, ArrayList<Integer[]> indexesToIgnore) {
		char[] contextChars = context.toCharArray();
		int index = startParen + 1;
		int open = 1;
		int close = 0;
		while (open > close) {
			/* if the current character is a ( and it's not inside a comment or literal */
			if (contextChars[index] == '(' && !inIgnoredText(index,indexesToIgnore)) {
				open++;
			}
			/* if the current character is a ) and it's not inside a comment or literal */
	...
	/**
	 * Determines if a character is inside any of the bounds of a list of blocks of text
	 * to ignore.
	 * @param charIndex the index of the character whose position is being checked
	 * @param ignoredIndex a list of items like [int startIgnoreIndex, int endIgnoreIndex]
	 * that specifies which indexes are supposed to be ignored
	 * @return returns true if the character index is within any of the ignore bounds, otherwise
	 * returns false.
	 */
	private static boolean inIgnoredText(int charIndex, ArrayList<Integer[]> ignoredIndexes) {
	...
	/**
	 * Build a list of character indexes that should be ignored during processing.
	 * Characters to be ignored are those inside of comments, string literals, 
	 * and character literals. This function returns the bounds of indexes of
	 * characters and literals in two separate lists.
	 * @param code the string to generate this list of bounds for
	 * @return returns two ArrayList<Integer[]> objects, wrapped inside of one ArrayList,
	 * with the format [comment indexes, literal indexes]. Then each of those elements is
	 * a list with the format [start ignore index, end ignore index].
	 */
	private static ArrayList<ArrayList<Integer[]>> getIgnoreIndexes(String code) {
		...
		/* List to hold our other two lists, so that the user can separate them at will */
		ArrayList<ArrayList<Integer[]>> finalValues = new ArrayList<ArrayList<Integer[]>>();

		/* iterate through character by character looking for comments and literals */
		char[] codeChars = code.toCharArray();
		int index = 0;
	 ...
	 * "Th/*comment*\/is line //other stuff" becomes "This line"
	 * Necessary so that line by line searching for beginning/ending with things can be accurate
	 * @param commented The String to remove comments from
	...
	private static String removeComments(String commented) {
		...
		while (index < commentedChars.length) {
			...
			if (commentedChars[index] == '"') {
				/* we don't want to remove something that looks like a comment if it's in here */
	...
	/**
	 * Removes all String/char literals from a String and return the stripped String
	 * "System.out.println("\"Hello,\" he said." + String.valueOf('\n'));" becomes "System.out.println( + String.valueOf());"
	 * Doesn't remove literals from comments, so "// Method "removeLiterals" removes literals" would be untouched
	 * Necessary so that we can reliably check which functions are present in user code
	 * @param stringWithLiterals The String to remove String/char literals from
	...
	private static String resourceRead(String resourcePath) {
		try {
			InputStream inputStream = Create.class.getResourceAsStream(resourcePath);
	...
	private static String fileRead(String filePath) {
		try {
			...
			System.out.println("Compiler encountered an error reading file \"" + filePath + "\"");
			e.printStackTrace();
			System.exit(1);
			return "null";
		}
	...
	private static boolean fileWrite( String fPath, String data ) {
		...
		try {
			...
			System.out.println("Compiler encountered an error writing to file \"" + fPath + "\"");
			e.printStackTrace();
	 ...
	 * @param directory File object representing directory to remove
	 ...
	 * @param userImport the user's import statement
	 * @param generatedImports the generated list of imports
	 * @return true if the userImport is a line inside of generatedImports
	 */
	private static boolean alreadyImported(String userImport, String generatedImports) {
		for (String line : generatedImports.split("\n")) {
	 ...
	 * @param newLines amount of newline characters to print at end of string
	 */
