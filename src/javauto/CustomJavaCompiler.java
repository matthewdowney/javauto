...
public class CustomJavaCompiler {
	public static DiagnosticCollector<JavaFileObject> compile(String name, String code, String outDir, String classpath) throws IOException {
		/* get JavaCompiler object to create the .class file */
		JavaCompiler compiler = ToolProvider.getSystemJavaCompiler();

		/* throw an exception if there's an issue with the compiler */
		if (compiler == null) {
			throw new NullPointerException("Couldn't find java system compiler.");
		}
		...
		fileManager.setLocation(StandardLocation.CLASS_OUTPUT, Arrays.asList(new File(outDir)));

		/* tell the file manager where to look for related class files that may be referenced */
		fileManager.setLocation(StandardLocation.CLASS_PATH, Arrays.asList(new File(classpath)));

		/* get a DiagnosticCollector<JavaFileObject> object to hold a list of potential errors */
		...
		/* compile it -- note that errors will be inserted into our diagnostics variable */
