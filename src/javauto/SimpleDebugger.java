...
public class SimpleDebugger {
	 ...
	 * @param userCode the user code to debug
	 * @return true only if there are no errors, false otherwise
	 */
	public static boolean debug(String userCode, String fileName) {
		/* set the global file name so errors can be specific about where an error happens */
	 ...
	 * @param generatedCode the string the character position is in
	...
	/**
	 * Make sure the python file String replacement is formatted like:
	 * String s = "Hello, my name is %s" % ("John")
	 * with the parenthesis after the %
	 * @param userCode the code we're auditing
	 */
	private static void checkStringReplaceFormat(String userCode) {
		...
		while (matcher.find()) {
			...
			if (!nextChar.equals("(")) {
				...
				String message = "Error in " + userCodeFileName + " line " + lineNum + "\n" + "\t" + userCode.split("\n")[lineNum-1] + "\n" + "Expected a ( after the %.\nLike \"Hello %s\" % (\"John\")";
	...
	private static void checkVariableDeclarations(String userCode, ArrayList<Integer[]> allIgnores) {
		...
		for (int i = 0; i < lines.length; i++) {
			...
			while (matcher.find()) {
				...
				if (lowerVars.contains(found.toLowerCase()) || found.toLowerCase().equals("args")) {
					errorLines.add(i+1);
					String message = "Error in " + userCodeFileName + " line " + (i+1) + "\n" + "\t" + userCode.split("\n")[i] + "\n" + "Cannot override \"" + found + "\", use a different name.";
	...
	private static void checkFunctionDeclarations(String userCode, ArrayList<Integer[]> allIgnores) {
	...
	private static void checkJavautoCase(String userCode, ArrayList<Integer[]> allIgnores) {
		...
		for (String s : funcs) {
			...
			while (matcher.find()) {
				...
				if (!foundFunc.equals(s) && foundFunc.toLowerCase().equals(s.toLowerCase()) && !inIgnoredText(index, allIgnores)) {
					int lineNum = getLineNumber(index, userCode);
					errorLines.add(lineNum);
					String message = "Error in " + userCodeFileName + " line " + lineNum + "\n" + "\t" + userCode.split("\n")[lineNum-1] + "\n" + "Case of \"" + foundFunc + "\" is incorrect, it should be \"" + s + "\".";
					errors.add( new String[] {String.valueOf(lineNum), message} );
				}
			}

			/* do the whole regex again looking for func at the beginning of file */
			...
			while (matcher.find()) {
				...
				if (!foundFunc.equals(s) && foundFunc.toLowerCase().equals(s.toLowerCase())) {
					int lineNum = 1;
					errorLines.add(lineNum);
					String message = "Error in " + userCodeFileName + " line " + lineNum + "\n" + "\t" + userCode.split("\n")[lineNum-1] + "\n" + "Case of \"" + foundFunc + "\" is incorrect, it should be \"" + s + "\".";
					errors.add( new String[] {String.valueOf(lineNum), message} );
	...
	private static ArrayList<String> getFunctions() {
		...
		for (String line: functionContentsLines) {
			...
			if ((line.trim().startsWith("public ") || line.trim().startsWith("private ")) && (line.trim().endsWith("{")) && (!line.trim().startsWith("public class"))) {
				...
				if (!functionName.toLowerCase().equals("run") && !functionDataList.contains(functionName.trim())) {
	...
	private static ArrayList<String> getJavauto() {
		...
		for (String line: functionContentsLines) {
			...
			/* if it's a variable */
			else if ( (line.trim().startsWith("public ") || line.trim().startsWith("private ")) && line.trim().endsWith(";")) {
				String[] parts = line.split("=")[0].split(" ");
				String name = parts[parts.length-1].trim();
	 ...
	 * @param startBracket the index of the '{'.
	 ...
	 */
	private static int findEndBracket(int startBracket, String context, ArrayList<Integer[]> indexesToIgnore) {
	...
	private static int findStartBracket(int endBracket, String context, ArrayList<Integer[]> indexesToIgnore) {
		char[] contextChars = context.toCharArray();
	 ...
	 * @return the index of the closing paren
	 */
	private static int findEndParen(int startParen, String context, ArrayList<Integer[]> indexesToIgnore, int lineNumber) {
		char[] contextChars = context.toCharArray();
		...
		try {
			...
			String message = "Error in " + userCodeFileName + " line " + (lineNumber + 1) + "\n" + "\t" + context + "\n" + "Unclosed '(', expected corresponding ')'";
			errors.add( new String[] {String.valueOf(lineNumber), message} );
		}
		return index;
	}
	private static int findStartParen(int endParen, String context, ArrayList<Integer[]> indexesToIgnore, int lineNumber) {
		char[] contextChars = context.toCharArray();
